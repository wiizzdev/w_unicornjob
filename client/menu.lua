local mainMenu = RageUI.CreateMenu("", "Unicorn Club")
			...
			while open do
                ...
                RageUI.IsVisible(annonces, function() 
                    RageUI.Button("Annonce d'Ouverture", nil, {RightLabel = "→"}, true , {
                        onSelected = function()
                        	TriggerServerEvent('unicorn:ouvert')
                        end
                    })		
					RageUI.Button("Annonce de Fermeture", nil, {RightLabel = "→"}, true , {
    					onSelected = function()
        					TriggerServerEvent('unicorn:ferme')
    					end
					})
					RageUI.Button("Annonce Personnalisée", nil, {RightLabel = "→"}, true , {
						onSelected = function()
							...
							if notif then
								TriggerServerEvent('unicorn:perso', notif)
							else
...
Keys.Register('F6', 'unicornjob', 'Ouvrir le menu Unicorn', function()
